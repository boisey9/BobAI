...
`codex/owner-access-activation` prepares a separately restricted Web database role and expands real PostgreSQL/browser acceptance to run through it. A fresh local encrypted production backup (ten tables) restored in five seconds. Upload and dedicated nightly storage credential creation are awaiting explicit automatic-review confirmations. See [owner-access preparation](docs/changes/2026-09-08-owner-access-preparation.md).
