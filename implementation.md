...
[PR #45](https://github.com/boisey9/BobAI/pull/45) on `codex/account-linking` adds owner-reviewed project OAuth at `/mcp/linked`, signed consent, PKCE, opaque-token introspection, revocation, private client provisioning, and recovery cleanup. The source review's pre-introspection throttling and URL-normalization findings are fixed; 139 Core tests and the repeated PostgreSQL protocol/provisioning/browser acceptance pass, including a concurrent limit shared by two gateways before verification. Web build and a 24-table restore previously passed. Final CI and production activation remain pending; migration 007 and OAuth flags remain unapplied/disabled. See [account-linking change](docs/changes/2026-09-07-project-account-linking.md) and [operator runbook](docs/account-linking.md).
