...
The source review identified invalid-token traffic bypassing the grant limiter and equivalent URL spellings failing exact claim checks. Core now normalizes issuer/resource URLs and always reserves capacity before network introspection: a PostgreSQL owner-wide 200/minute bucket shared across instances, independent of the optional per-grant limiter. Rotating bearer strings or forwarding headers cannot create new attempt buckets. Exhaustion returns 429 with Retry-After; capacity failure returns a sanitized 503 without contacting Web.
...
- Core typecheck, 139 regression tests across 29 files and three import dry-runs passed after the review fixes. Web production build/typecheck previously passed; the merge retained its identical source.
...
- The September 8 repeat passed the full protocol/provisioning/browser suite and the new mandatory attempt limit against PostgreSQL: with one of 200 slots remaining, six concurrent rotating invalid bearers across two gateways admitted exactly one introspection and returned five 429 responses with Retry-After. Spoofed forwarding headers did not create capacity. Unit checks cover capacity-store failure without an outbound call and uppercase/default-port URL normalization.
...
- Served resource/authorization metadata also passed local discovery, including the canonical issuer, S256 and disabled dynamic registration; the browser followed the discovered authorization endpoint.
...
PR #44 merged as `9833e22` after owner approval and both production deployables passed source/health verification on September 8 (Toronto). Local HTTP staging remains the accepted OAuth test environment after a separate staging-secret transfer was rejected. Nightly backups remain disabled until the dedicated storage credential and first production backup/restore pass.
